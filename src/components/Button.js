...
const Button = ({
  ...
  const getVariantStyle = () => {
    switch (variant) {
      case 'secondary':
        return { bg: colors.button.secondary, text: colors.button.secondaryText, border: colors.glassBorder };
      case 'danger':
      ...
      {loading ? (
        ...
        <View style={styles.content}>
          {icon && iconPosition === 'right' && (
            <Ionicons name={icon} size={20} color={disabled ? colors.button.disabledText : variantStyle.text} style={{ marginLeft: 8 }} />
          )}
          <Text style={[
            styles.text,
          ...
          {icon && iconPosition === 'left' && (
            <Ionicons name={icon} size={20} color={disabled ? colors.button.disabledText : variantStyle.text} style={{ marginRight: 8 }} />
          )}
        </View>
      )}
    </TouchableOpacity>
